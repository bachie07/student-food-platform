...
const Navbar = () => {
    ...
    return(
        ...
        <nav className={`sticky top-0 z-50 bg-red-800 text-white px-6 py-1 shadow-md transition-all duration-500 sm:${isScrolled ? "py-1" : "py-6"}`}>
                ...
                <div className="max-w-full mx-auto flex-col space-y-3 mt-2">
                    ...
                    <div className={`flex sm:${ isScrolled ? "mb-1" : "mb-5" }`}>
                        ...
                        <Menu className="block sm:hidden md:hidden lg:hidden"/>
                        </div>
                        
                        <div className="w-1/3 flex justify-center">
                        <Link to="/" className={`text-lg font-bold transition-all duration-500 font-serif sm:${ isScrolled ? "text-3xl" : "text-5xl"}`}>
                        uniMunch
                        </Link>
                        </div>
                    ...
                    {!token ? (
                        ...
                        <Link to="/login" className="hidden sm:block ml-auto hover:text-gray-300 transition-colors text-[15px] mr-10">  Sign in </Link>
                        </div>
                    ) : ( 
                        <div className="w-1/3 flex justify-end">
                    ...
                    <hr className={`hidden sm:block h-px bg-white border-0 transition-all duration-500 sm:${ isScrolled ? "my-4" : "my-7"}`}/>
                    
                    <div className="flex">

                        <div className="hidden sm:block sm:w-1/2 md:w-1/3 sm:justify-start sm:space-x-5 ml-10 sm:text-md md:text-lg">

                            {/* Make sure to match the link address to the ones defined in app.jsx*/}

                            <Link to="/recipes" className="hover:text-gray-300 transition-colors font-semibold"> Recipes </Link>

                            <Link to="/groceries" className="hover:text-gray-300 transition-colors font-semibold "> Grocery </Link>

                            <Link to="/eats" className="hover:text-gray-300 transition-colors font-semibold">  Eats </Link>
